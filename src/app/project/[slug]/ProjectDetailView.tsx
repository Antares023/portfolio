...
export default function ProjectDetailView({ project }: { project: any }) {
  ...
  useEffect(() => {
    if (activeTab === 'overview' && project?.screenshots && project.screenshots.length > 1) {
  ...
  const mainLang = project.tags && project.tags.length > 0 ? project.tags[0] : 'JavaScript';

  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      {/* ── Breadcrumb ────────────────────────────────────────── */}
      <div className="flex items-center gap-2 text-sm mb-6">
        <Link href="/" className="text-accent-blue hover:underline font-mono">
          Antares023
        </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3">
          ...
          <div className="mb-8">
            {activeTab === 'code' ? (
              <CodeBlock
                code={project.codeSnippet || '// No code provided'}
                language={project.codeLanguage || 'javascript'}
                filename={`src/main.${project.codeLanguage === 'javascript' ? 'js' : project.codeLanguage === 'python' ? 'py' : project.codeLanguage === 'cpp' ? 'cpp' : 'ts'}`}
              />
            ) : (
                ...
                {project.screenshots && project.screenshots.length > 0 ? (
                  <div className="overflow-hidden relative w-full rounded border border-gh-border shadow-md bg-gh-canvas group">
                    ...
                    <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-10">
                      {project.screenshots.map((_: any, idx: number) => (
                        <button
                          key={idx}
